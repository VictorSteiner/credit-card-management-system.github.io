...
export class CreditCardAddComponent {
	...
	onSubmit(formGroup): void {
		const creditCard: CreditCard = {
			card_number: parseInt(formGroup.card_number, 10),
			csc_code: parseInt(formGroup.csc_code, 10),
			cardholder_name: formGroup.cardholder_name,
			expiration_date_month: parseInt(formGroup.expiration_date_month, 10),
			expiration_date_year: parseInt(formGroup.expiration_date_year, 10),
	...
	createForm(): void {
		this.formGroup = this.fb.group({
			...
			issuer: ['', [Validators.required, Validators.pattern('^[a-zA-Zs]*$')]],
		});
	}
	...
	get errorMonth(): string {
		const form: FormControl = this.formGroup.get(
			'expiration_date_month'
		) as FormControl;
		return form.hasError('required')
			? 'Month is required'
